...
commands.main = {
	...
	{"ban", function(msg) if string.len(msg.content) < 30 then msg:reply(storage.noreason) return end if class.admin.isAuthorAdmin(msg.author.id,admins) or class.admin.isAuthorMestima(msg.author.id) then local pos = string.find(msg.content," ",6) local id = string.sub(msg.content,6,pos-1) class.admin.banId(id,msg) msg.channel:send("```ID: "..id.." banned.```") end end,4},
	{"unban", function(msg) if string.len(msg.content) < 30 then msg:reply(storage.noreason) return end if class.admin.isAuthorAdmin(msg.author.id,admins) or class.admin.isAuthorMestima(msg.author.id) then local pos = string.find(msg.content," ",8) local id = string.sub(msg.content,8,pos-1) class.admin.unbanId(id,msg) msg.channel:send("```ID: "..id.." unbanned.```") end end,6},
